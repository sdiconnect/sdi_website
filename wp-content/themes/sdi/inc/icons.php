<?php
/**
 * Lucide icon paths (inner SVG markup), 2px stroke, currentColor.
 * Generated from lucide-static v1.24.0 (ISC). Social glyphs appended manually.
 * @package SDi
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

function sdi_icon_paths() {
  return array(
    ...
    // Brand glyphs (not shipped by Lucide) — filled paths, use fill=currentColor via .sdi-icon--brand.
    ...
    // Filled star (for ratings) — solid fill, no stroke.
    'star-solid' => '<path fill="currentColor" stroke="none" d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />',
  );
}

/**
 * Render a row of filled rating stars.
 *
 * @param int    $count Number of filled stars (default 5).
 * @param int    $size  Star pixel size.
 * @param string $color CSS color (default amber). Pass '' to inherit currentColor.
 * @return string
 */
function sdi_stars( $count = 5, $size = 16, $color = 'var(--amber-500)' ) {
	$count = max( 0, min( 5, (int) $count ) );
	$style = $color ? ' style="color:' . esc_attr( $color ) . ';"' : '';
	$out   = '<span class="sdi-stars" aria-hidden="true"' . $style . '>';
	for ( $i = 0; $i < $count; $i++ ) {
		$out .= sdi_icon( 'star-solid', $size );
	}
	$out .= '</span>';
	return $out;
}
...
/**
 * Render a Lucide icon inline. Inherits currentColor.
 *
 * @param string $name  Icon name.
 * @param int    $size  Pixel size (width & height).
 * @param array  $args  Optional: 'class', 'stroke' (width), 'label' (aria-label).
 */
function sdi_icon( $name, $size = 24, $args = array() ) {
	static $paths = null;
	if ( null === $paths ) {
		$paths = sdi_icon_paths();
	}
	if ( empty( $paths[ $name ] ) ) {
		return '';
	}
	$size   = (int) $size;
	$class  = 'sdi-icon' . ( isset( $args['class'] ) ? ' ' . $args['class'] : '' );
	$stroke = isset( $args['stroke'] ) ? (float) $args['stroke'] : 2;
	$aria   = isset( $args['label'] ) ? ' role="img" aria-label="' . esc_attr( $args['label'] ) . '"' : ' aria-hidden="true"';
	...
	return sprintf(
		...
		esc_attr( $stroke ),
		$aria,
		$paths[ $name ] // phpcs:ignore WordPress.Security.EscapeOutput -- trusted static SVG markup.
	);
}

/** Echo helper. */
function sdi_the_icon( $name, $size = 24, $args = array() ) {
	echo sdi_icon( $name, $size, $args ); // phpcs:ignore WordPress.Security.EscapeOutput -- sdi_icon returns safe SVG.
}
