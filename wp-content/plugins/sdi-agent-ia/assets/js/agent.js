...
(function () {
  ...
  panel.innerHTML =
    ...
    '<div class="sdi-ai__foot">' +
      ...
      '</div>' +
      '<div class="sdi-ai__actions"><button type="button" class="sdi-ai__link" data-transcript>Envoyer la conversation par e-mail</button></div>' +
    '</div>';
  ...
  function showLeadForm() {
    ...
    var fields = cfg.collectEmail
      ? '<label>Vos coordonnées pour être recontacté</label>' +
        '<input type="text" data-name placeholder="Votre nom">' +
        '<input type="email" data-email placeholder="Votre e-mail">'
    ...
    wrap.querySelector('[data-send-transcript]').addEventListener('click', function () {
      ...
      var extra = { page: window.location.href };
      if (nameEl) { extra.name = nameEl.value; }
      if (mailEl) { extra.email = mailEl.value; }
      wrap.querySelector('[data-send-transcript]').disabled = true;
      post('sdi_ai_transcript', extra).then(function (res) {
        wrap.remove();
        var msg = (res && res.success && res.data && res.data.message) ? res.data.message
